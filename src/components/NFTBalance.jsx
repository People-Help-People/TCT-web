...
import { Card, Image, Skeleton, Button } from "antd";
  ...
  const requestNFTVerification = async (nft, token_id) => {
    ...
    let options = {
      ...
      abi: [
        {
          inputs: [
            { internalType: "string", name: "_queryParams", type: "string" },
          ],
    ...
    const fetchNFTs = async () => {
      if (account && !loading) {
        ...
        const allNFTs = await Promise.all(
          NFTBalances.map(async (NFTBalance) => {
            const hydratedNFTs = await Promise.all(
              NFTBalance.balance.map(async (nft) => {
                const status = await fetchNFTstatus(
                  nft.token_address,
                  nft.token_id
                );
                if (!nft.metadata) {
    ...
    <div style={{ padding: "15px", maxWidth: "1030px", width: "100%" }}>
      <div>
        <Skeleton loading={loading}>
          {allChainNFTs.length !== 0 &&
            allChainNFTs.map(
              (chain) =>
                chain.balance.length !== 0 && (
                  <div key={chain.chain}>
                    <h1>{chain.chain.toUpperCase()}</h1>
                    <div style={styles.NFTs}>
                      {chain.balance.map((nft, index) => {
                        return (
                          <Card
                            hoverable
